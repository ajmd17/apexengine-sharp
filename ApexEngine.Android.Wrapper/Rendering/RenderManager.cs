...
namespace ApexEngine.Rendering
{
    public class RenderManager
    {
        ...
        public void RenderDepthTexture(Environment env, Camera cam)
        {
            ...
            RenderBucketDepth(env, cam, Bucket.Opaque);
            RenderBucketDepth(env, cam, Bucket.Transparent);
        ...
        public void RenderBucketDepth(Environment env, Camera cam, Bucket bucket)
        {
            for (int i = 0; i < geometries.Count; i++)
            {
                if (geometries[i].AttachedToRoot && geometries[i].Bucket == bucket)
